...
import { Permit2, DexpertUniversalRouter } from '../../typechain'
...
describe('Uniswap V2 and V3 Tests:', () => {
  ...
  let router: DexpertUniversalRouter
  let permit2: Permit2
  ...
  beforeEach(async () => {
    ...
    router = (await deployUniversalRouter(permit2)).connect(bob) as DexpertUniversalRouter
    owner = await router.owner()
    console.log("router owner:", owner)
    console.log("alice:",alice.address)
    console.log("bob:",bob.address)
    // await router.connect(bob).setFeeBps(1, 0, 2);
  ...
  describe('Trade on UniswapV2', () => {
    const amountIn: BigNumber = expandTo18DecimalsBN(7)
    beforeEach(async () => {
    ...
    describe('ERC20 --> ERC20', () => {
      it('completes a V2 exactIn swap', async () => {
        ...
        await wethContract.connect(bob).approve(router.address, amountIn)
        const minAmountOut = expandTo18DecimalsBN(0.0001)
        // planner.addCommand(CommandType.APPROVE, [
        //   DAI.address,
        //   router.address,
        // ])
    ...
    describe.skip('ETH --> ERC20', () => {
      it('completes a V2 exactIn swap', async () => {
        ...
        planner.addCommand(CommandType.WRAP_ETH, [pairAddress, amountIn, true, 0])
      ...
      it('completes a V2 exactOut swap', async () => {
        ...
        planner.addCommand(CommandType.WRAP_ETH, [ADDRESS_THIS, value, true, 0])
        planner.addCommand(CommandType.V2_SWAP_EXACT_OUT, [
          MSG_SENDER,
          amountOut,
  ...
  describe.skip('Trade on UniswapV3', () => {
    const amountIn: BigNumber = expandTo18DecimalsBN(5000)
    ...
    describe('ERC20 --> ERC20', () => {
      it('completes a V3 exactIn swap', async () => {
        ...
        addV3ExactInTrades(planner, 1, 0)
      ...
      it('completes a V3 exactOut swap', async () => {
        ...
        planner.addCommand(CommandType.V3_SWAP_EXACT_OUT, [MSG_SENDER, amountOut, amountInMax, path, SOURCE_MSG_SENDER, true, 0])
    ...
    describe('ETH --> ERC20', () => {
      it.skip('completes a V3 exactIn swap', async () => {
        const tokens = [WETH.address, DAI.address]
        // const tokens = ["0xB72bC8971D5e595776592E8290bE6F31937097c6", "0xfFf9976782d46CC05630D1f6eBAb18b2324d6B14"]
        const amountOutMin: BigNumber = expandTo18DecimalsBN(0.0005)

        planner.addCommand(CommandType.WRAP_ETH, [ADDRESS_THIS, amountIn, false, 0])
      ...
      it('completes a V3 exactOut swap', async () => {
        ...
        planner.addCommand(CommandType.WRAP_ETH, [ADDRESS_THIS, amountInMax, false, 0])
        planner.addCommand(CommandType.V3_SWAP_EXACT_OUT, [MSG_SENDER, amountOut, amountInMax, path, SOURCE_ROUTER, true, 0])
        planner.addCommand(CommandType.UNWRAP_WETH, [MSG_SENDER, 0])
  ...
  function encodePathExactOutput(tokens: string[]) {
    return encodePath(tokens.slice().reverse(), new Array(tokens.length - 1).fill(3000))
  }
})