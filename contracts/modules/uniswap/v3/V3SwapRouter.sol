...
abstract contract V3SwapRouter is UniswapImmutables, Permit2Payments, IUniswapV3SwapCallback {
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external {
        ...
        (, address payer, uint256 level, uint256 swapType) = abi.decode(data, (bytes, address, uint256, uint256));
        ...
        if (isExactInput) {
            ...
            payOrPermit2Transfer(tokenIn, payer, msg.sender, amountToPay, level, swapType);
        } else {
            // either initiate the next swap or pay
            if (path.hasMultiplePools()) {
                // this is an intermediate step so the payer is actually this contract
                path = path.skipToken();
                _swap(-amountToPay.toInt256(), msg.sender, path, payer, false, level, swapType);
            } else {
                if (amountToPay > maxAmountInCached) revert V3TooMuchRequested();
                // note that because exact output swaps are executed in reverse order, tokenOut is actually tokenIn
                payOrPermit2Transfer(tokenOut, payer, msg.sender, amountToPay, level, swapType);
            }
        }
    }
    ...
    function v3SwapExactInput(
        ...
        (address tokenIn, uint24 fee, address tokenOut) = path.decodeFirstPool();
        uint256 amountToPay = payOrPermit2TransferFee(tokenIn, payer, msg.sender, uint256(amountIn), level, swapType);
        uint256 amountOut;
        while (true) {
            ...
            amountToPay = uint256(-(zeroForOne ? amount1Delta : amount0Delta));
    ...
    function _swap(
        ...
        (amount0Delta, amount1Delta) = IUniswapV3Pool(computePoolAddress(tokenIn, tokenOut, fee)).swap(
            ...
            abi.encode(path, payer, 0, swapType)
        );
    }
